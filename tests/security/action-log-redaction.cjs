...
const test = path.join(root, 'tests/php/action-log-redaction.php');
...
if (result.error?.code === 'ENOENT') {
	...
	if (container.error || !container.stdout.trim()) {
		throw Error('Action log redaction tests need PHP CLI or a running `docker compose` snappymail service.');
	}
	result = run('docker', [
		'compose', 'exec', '-T',
